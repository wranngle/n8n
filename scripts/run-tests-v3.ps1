...
Write-Host "Found $($testsResponse.tests.Count) tests"

# Build test array with correct field name 'id'
$testArray = @()
foreach ($test in $testsResponse.tests) {
    $testArray += @{ test_id = $test.id }
}
...
$body = @{ tests = $testArray } | ConvertTo-Json -Depth 5
...
try {
    ...
    Write-Host ""
    Write-Host "Polling for results..." -ForegroundColor Cyan
    ...
    while (-not $complete -and $attempt -lt $maxAttempts) {
        ...
        try {
            ...
            Write-Host "[$attempt] Poll error" -ForegroundColor Red
        }
    }
    ...
    if ($complete) {
        ...
        Write-Host ""
        if ($rate -eq 100) {
            Write-Host "Pass Rate: $rate% - PERFECT!" -ForegroundColor Green
        } elseif ($rate -ge 90) {
            Write-Host "Pass Rate: $rate% - EXCELLENT!" -ForegroundColor Green
        } elseif ($rate -ge 80) {
            Write-Host "Pass Rate: $rate% - GOOD" -ForegroundColor Yellow
        } else {
            Write-Host "Pass Rate: $rate% - NEEDS WORK" -ForegroundColor Red
        }
        Write-Host "========================================" -ForegroundColor Cyan
        ...
        Write-Host "Timeout - check invocation: $invocationId" -ForegroundColor Yellow
    }

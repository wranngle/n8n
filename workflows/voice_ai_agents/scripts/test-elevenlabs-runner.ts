...
async function main() {
  ...
  const test1: TestCase = {
    test_id: 'TEST-EL-001',
    type: 'elevenlabs',
    name: 'Sarah responds to greeting',
    description: 'Test that Sarah responds appropriately to a simple greeting',
    input: {
      agent_id: SARAH_AGENT_ID,
      test_prompt: "Hi, I'm interested in learning more about your services.",
    },
    expected_output: {
      // Sarah identifies as Wranngle Digital Dispatcher
      response_contains: ['wranngle'],
      // Sarah's discovery conversations can go 20+ turns
      max_turns: 25,
    },
    tags: ['smoke', 'greeting'],
    enabled: true,
  ...
  const result = await runner.execute(test1, { timeout: 120000 });

...
- [ ] 1.3 Get detailed metadata for key workflows
  - For each workflow in agent-registry.yaml (sarah-send-sms, post-call, etc.)
...
- [ ] 2.2 Analyze agent configs
  - Compare `agents/sarah/tech-spec.md` to cloud agent prompt
  - Run `mcp__elevenlabs-mcp__get_agent` for agent_8001kdgp7qbyf4wvhs540be78vew
  - Check if local prompt matches cloud
  - Document differences

- [ ] 2.3 Identify test dependencies
  - Find all references to `agents/sarah/tests/scenarios.yaml`
  - Confirm only supersystem/ uses it
  - Plan migration path to `supersystem/scenarios/sarah.yaml`
...
- [ ] 3.2 Move test scenarios
  - Copy `agents/sarah/tests/scenarios.yaml` → `supersystem/scenarios/sarah.yaml`
...
- [ ] 5.4 Run full-text search for broken references
  - `rg "agents/sarah" --type md --type yaml --type json`
...
- [ ] supersystem/scenarios/sarah.yaml exists with correct content

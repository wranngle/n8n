...
**Key Stakeholders:**
- Wranngle Systems engineers managing production agents
...
**Observation**: `agents/sarah/tests/scenarios.yaml` is used by supersystem testing framework

**Options Considered:**
1. **supersystem/scenarios/sarah.yaml** - Co-locate with testing framework
2. **tests/scenarios/sarah.yaml** - Separate top-level tests/ directory
3. **Keep in agents/sarah/** - Maintain current location

**Choice**: supersystem/scenarios/sarah.yaml (Option 1)
...
2. Copy test scenarios: `agents/sarah/tests/scenarios.yaml` → `supersystem/scenarios/sarah.yaml`
...
git show <commit>:agents/sarah/tech-spec.md > restored.md
```

...
**Before:**
- ❌ All callers get generic greeting: "Hi, this is Sarah from Wranngle"
...
- ✅ Pre-call data enrichment from Pipedrive + Google Sheets
...
```
Inbound Call → ElevenLabs → Webhook (n8n) → [Pipedrive + Sheets] → Enrich → Agent
                   ↓
...
1. **n8n Webhook:** `/client-initiation-data`
   ...
   - Queries: Pipedrive (person lookup), Google Sheets (call history)
   - Returns: `conversation_initiation_client_data` JSON
   - Timeout: <500ms (P95)

2. **Dynamic Variables:**
   - Regular: `customer_name`, `company`, `account_tier`, `call_history`
   - Secret: `secret__pipedrive_person_id`, `secret__google_sheet_row`
...
3. **Agent Enhancements:**
   ...
   - SMS tool auto-populated with secret Pipedrive ID
...
3. n8n looks up caller:
   - Pipedrive: "John Smith, Acme Corp, Gold tier"
...
- Conflicts resolved: Most recent data wins (if timestamps), otherwise Pipedrive > Sheets

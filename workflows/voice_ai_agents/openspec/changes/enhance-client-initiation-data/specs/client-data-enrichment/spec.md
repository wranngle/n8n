...
- **GIVEN** an inbound call from a known Pipedrive contact
...
- **GIVEN** an inbound call from an unknown phone number (not in Pipedrive or Sheets)
...
- **GIVEN** a webhook call with `agent_id` that does not match Sarah's ID
...
#### Scenario: Successful parallel lookup from Pipedrive and Google Sheets
- **GIVEN** a known caller with data in both Pipedrive and Google Sheets
- **WHEN** the webhook performs lookups
- **THEN**:
  - Pipedrive API call initiated within 10ms of webhook receipt
...
#### Scenario: Pipedrive timeout triggers fallback to Sheets
- **GIVEN** Pipedrive API is slow (>300ms) but Sheets is responsive
- **WHEN** the webhook performs lookups
- **THEN**:
  - Pipedrive lookup times out at 300ms
...
- **GIVEN** both Pipedrive and Sheets APIs are unresponsive
- **WHEN** the webhook performs lookups
- **THEN**:
...
- **GIVEN** a Pipedrive person is found
- **WHEN** dynamic variables are constructed
- **THEN**:
  - Secret variables use `secret__` prefix: `secret__pipedrive_person_id`, `secret__google_sheet_row`
  - Secret variables are NOT sent to the LLM (never in prompt context)
  - Secret variables ARE available in tool parameter defaults: `{{secret__pipedrive_person_id}}`
...
When data is available from multiple sources, the webhook SHALL prioritize Pipedrive as the authoritative source of truth.

#### Scenario: Conflicting names between Pipedrive and Sheets
- **GIVEN** Pipedrive returns name "John Smith" and Sheets returns "John S."
- **WHEN** the webhook merges data
- **THEN**:
  - `dynamic_variables.customer_name = "John Smith"` (Pipedrive wins)
  - `dynamic_variables.data_source = "pipedrive"`
  - Sheets data is used only for fields not in Pipedrive (e.g., `call_history`)

#### Scenario: Sheets-only data when Pipedrive has no match
- **GIVEN** Pipedrive returns no results but Sheets has call history
- **WHEN** the webhook merges data
- **THEN**:
...
- **GIVEN** Pipedrive has no `account_tier` custom field set
...
- **GIVEN** Pipedrive returns `account_tier = "Gold"` for the caller
- **WHEN** the webhook constructs the response
- **THEN**:
  - `conversation_config_override.agent.first_message` is populated with VIP greeting
  - First message template: `"Hi {{customer_first_name}}, this is Sarah from Wranngle. I see you're one of our premium clients - how can I help you today?"`
...
- **AND** Pipedrive returns no special tier (defaults to Bronze)
- **WHEN** the webhook constructs the response
- **THEN**:
  - `conversation_config_override.agent.first_message` is populated with returning customer greeting
  - First message template: `"Hi {{customer_first_name}}, great to hear from you again! This is Sarah from Wranngle."`
...
- **THEN** the following is logged to n8n execution log:
  ...
  - Pipedrive lookup time (ms) and result (success/timeout/error)
  - Sheets lookup time (ms) and result
  - Data source used (pipedrive/sheets/cache/none)
...
### Requirement: SMS tool must automatically include Pipedrive person ID

The `send_sms` tool SHALL leverage secret variables to automatically pass CRM context without requiring LLM knowledge.

#### Scenario: SMS tool call includes pipedrive_id from secret variable
- **GIVEN** the agent decides to send an SMS booking link
- **AND** `dynamic_variables.secret__pipedrive_person_id = 12345`
- **WHEN** the agent invokes the `send_sms` tool
- **THEN**:
  - Tool parameter `pipedrive_id` is auto-populated with value `12345`
  - Agent/LLM never sees or mentions the ID
  - SMS webhook (n8n) receives `pipedrive_id` in request body
  - SMS webhook can automatically create Pipedrive note: "SMS sent with booking link"

#### Scenario: SMS tool call with unknown caller (no Pipedrive ID)
- **GIVEN** `dynamic_variables.secret__pipedrive_person_id = 0` (unknown caller)
- **WHEN** the agent invokes `send_sms` tool
- **THEN**:
  - Tool parameter `pipedrive_id = 0` (null value)
  - SMS webhook receives `pipedrive_id = 0` and skips CRM integration
...
6. ✅ SMS tool automatically includes Pipedrive ID (validated via n8n logs)

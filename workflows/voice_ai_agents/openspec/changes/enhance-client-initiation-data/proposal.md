...
2. **CRM data integration** - No Pipedrive/Google Sheets lookup at call start
...
2. **Enriches conversations** with caller context from CRM (Pipedrive) and logs (Google Sheets)
...
| Secret Variables | ❌ Not configured | ✅ Used for IDs/tokens (secret__pipedrive_id) |
...
- **Secret variables**: `{{secret__pipedrive_id}}` - Hidden from LLM, available in tool calls
...
1. **Test webhook exists** (`supersystem/client-data-lookup-test.json`) but:
   ...
   - Missing production data sources (Pipedrive, Sheets)
...
2. **Post-call webhook** (`elevenlabs-post-call-bulletproof-v2.json`) references `dynamic_variables`:
   ...
   - Contains `pipedrive_person_id` but set post-call
...
4. **Sarah agent** (`agent_8001kdgp7qbyf4wvhs540be78vew`):
...
- **Pipedrive API** - Lead/contact data
...
2. **CRM priority** ✅ RESOLVED
   ...
   - **Priority for conflicts:** Most recent data wins (if timestamps available), otherwise Pipedrive > Sheets

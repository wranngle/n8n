...
sarah-wranngle:
  ...
  dynamic_variables:
    ...
    - name: secret__pipedrive_person_id
      type: number
      description: "Pipedrive person ID (hidden from LLM)"
  client_lookup_webhook: "https://n8n.wranngle.com/webhook/client-initiation-data"
```
...
- **Action:**
  ...
  4. Set webhookId: `client-initiation-data-sarah`
...
- **Action:**
  ...
  2. Validate: `agent_id` matches Sarah's ID
...
#### Task 2.3: Implement Pipedrive Lookup
- **Node Type:** `n8n-nodes-base.httpRequest`
- **Action:**
  1. Configure Pipedrive API credentials
...
- **Action:**
  1. Implement merge logic: Pipedrive > Sheets > Defaults
...
- **Logic:**
  ```javascript
  const pipedrive = $node["Pipedrive Lookup"].json;
  const sheets = $node["Google Sheets Lookup"].json;

  const dynamicVars = {
    customer_name: pipedrive?.name || sheets?.name || "there",
    customer_first_name: pipedrive?.name?.split(' ')[0] || "there",
    company: pipedrive?.org_name || "",
    account_tier: pipedrive?.account_tier || (sheets?.call_count > 5 ? "Silver" : "New"),
    call_history: sheets?.last_interaction || "First-time caller",
    interaction_count: sheets?.call_count || 0,
    notes: sanitize(sheets?.notes || ""),
    secret__pipedrive_person_id: pipedrive?.id || 0,
    secret__google_sheet_row: sheets?.row || 0,
    lookup_success: !!(pipedrive || sheets),
    data_source: pipedrive ? "pipedrive" : (sheets ? "sheets" : "none")
  };
  ...
  ```
- **Verification:**
  - Test with Pipedrive data → correct mapping
...
- **Action:**
  ...
  3. Add `pipedrive_id` parameter with default: `{{secret__pipedrive_person_id}}`
- **Tool Schema:**
  ```json
  {
    "name": "send_sms",
  ...
  ```
- **Verification:** Test SMS call → n8n receives pipedrive_id automatically
...
- **Action:**
  ...
  3. Enter webhook URL: `https://n8n.wranngle.com/webhook/client-initiation-data`
...
- **Test Case:** Call from known Pipedrive contact
- **Steps:**
  1. Identify test contact: John Smith (+15551234567) in Pipedrive
  2. Make test call to Sarah's number
  3. Verify agent greets: "Hi John" (or similar)
  4. Check n8n logs: Pipedrive lookup succeeded
...
- **Steps:**
  1. Create test VIP contact in Pipedrive (account_tier = "Gold")
  2. Make test call
...
- **Test Case:** Successful SMS booking with auto-populated Pipedrive ID
- **Steps:**
  1. Call from known contact
  2. Progress to SMS booking
  3. Check SMS webhook logs in n8n
  4. Verify `pipedrive_id` present in request (not asked by agent)
- **Verification:** SMS tool receives pipedrive_id, Pipedrive note created automatically
...
- **Test:** Simulate Pipedrive/Sheets API failures
- **Steps:**
...
- Task 2.3 and 2.4 (Pipedrive and Sheets lookups) can be built in parallel

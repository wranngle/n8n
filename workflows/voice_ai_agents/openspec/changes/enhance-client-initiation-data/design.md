...
│  URL: https://n8n.wranngle.com/webhook/      │
...
└──────┬───────────────────────────────────────┘
       ...
       ▼                  ▼                      ▼
┌─────────────┐    ┌─────────────┐      ┌─────────────┐
│  Pipedrive  │    │   Google    │      │    Cache    │
│  CRM API    │    │   Sheets    │      │   (Redis)   │
│             │    │  Call Logs  │      │  Optional   │
└─────────────┘    └─────────────┘      └─────────────┘
       │                  │                      │
       │ 4. Merge data (Pipedrive > Sheets > Cache)
       │                  │                      │
       └──────────────────┴──────────────────────┘
                          │
                          ▼
...
4A. Pipedrive Lookup (n8n-nodes-base.httpRequest)
...
5. Merge & Transform (n8n-nodes-base.code)
   ├─ Merge strategy: Combine all sources with conflict resolution (most recent or Pipedrive > Sheets)
...
```json
{
  ...
  "dynamic_variables": {
    ...
    "secret__pipedrive_person_id": "number",
    "secret__pipedrive_org_id": "number",
    "secret__google_sheet_row": "number",

    // System context
    "lookup_success": "boolean",
    "data_source": "string"  // "pipedrive" | "sheets" | "cache" | "none"
  },
  "conversation_config_override": {
    "agent": {
...
- **Secret variables**: `secret__pipedrive_person_id`
  - Prefix: `secret__`
  - Never sent to LLM
...
```markdown
# IDENTITY
You are Sarah, a lead specialist for Wranngle's "The 24/7 Filter"...
...
if (data.pipedrive?.is_vip) {
  return {
    first_message: `Hi {{customer_first_name}}, this is Sarah from Wranngle. I see you're one of our premium clients - how can I help you today?`
  };
} else if (data.sheets?.call_count > 2) {
  return {
    first_message: `Hi {{customer_first_name}}, great to hear from you again! This is Sarah from Wranngle.`
  };
} else {
...
**Benefits:**
- ✅ SMS webhook receives `pipedrive_id` automatically
- ✅ No need to re-lookup caller in CRM
- ✅ Secure (LLM never sees the ID)
- ✅ Enables automatic Pipedrive note creation
...
| **Pipedrive lookup** | 300ms | Try Sheets |
...
1. **Secret variables** for all IDs (Pipedrive, Sheet row numbers)
...
| **Pipedrive API latency** | P95: <250ms | n8n node timing |
...
2. Pipedrive API failure → verify fallback to Sheets
...
4. SMS tool call → verify pipedrive_id passed correctly
...
- Test with real calls to Sarah's dev number
- Monitor for 3 days

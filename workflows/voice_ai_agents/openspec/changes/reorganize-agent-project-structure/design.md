...
The voice_ai_agents project powers Wranngle Systems' production "24/7 Filter" AI hotline service. It began as exploratory development for a single agent (Sarah) and evolved into a multi-agent platform with comprehensive testing infrastructure (supersystem), n8n workflow orchestration, and ElevenLabs voice integration.

**Historical Evolution:**
1. **Phase 1 (Dec 2024)**: Initial Sarah agent development with rapid iteration on prompts
2. **Phase 2 (Dec-Jan 2025)**: SEWY Garage agent experimentation (never reached production)
...
**Key Stakeholders:**
- Wranngle Systems engineers maintaining production agents
...
### Decision 2: Agent Registry SEWY Entry

**Options Considered:**
1. **Remove entry** - Delete sewy-garage from agent-registry.yaml
2. **Mark inactive** - Add `status: INACTIVE` field
3. **Create skeleton** - Create agents/sewy-garage/ with README explaining status

**Choice**: Remove entry (Option 1)

**Rationale:**
- SEWY agent has no active development (no phone number, no active config directory)
...
- Mitigation: Decision log can document SEWY as historical experiment

### Decision 3: Sarah Prompt Files Consolidation

**Observation**: 4 standalone Sarah prompt files exist in old/:
- sarah-enhanced-prompt-v1.1.md (17KB)
- sarah-final-prompt-100pct.md (14KB)
- sarah-merged-prompt.md (8KB)
- (Plus agents-sarah-archive/ with 3 more variations)

**Current Source of Truth**: agents/sarah/tech-spec.md (846 lines, comprehensive)
...
|-----------|----------------|---------------------|
| Sarah Agent Config | agents/sarah/ | old/agents-sarah-archive/ |
| Sarah System Prompt | agents/sarah/tech-spec.md | old/sarah-*.md (4 files) |
| SMS Tool Definition | agents/sarah/tech-spec.md | old/sarah-sms-tool.json |
...
**Likelihood**: Low - comprehensive review showed all active info in agents/sarah/tech-spec.md
...
**Q2**: Should we create agents/sewy-garage/ as a placeholder?
**A2**: No - agent-registry.yaml removal is sufficient. If SEWY is revived, create the directory then.

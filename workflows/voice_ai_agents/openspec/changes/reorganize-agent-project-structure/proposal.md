...
The voice_ai_agents project has accumulated 100+ archived files scattered across multiple directories, creating organizational complexity and developer confusion. Active production files (agents/sarah/, pipelines/, supersystem/) are mixed with obsolete artifacts in the `old/` directory, which contains:
- **8 files** of superseded Sarah agent configs (agents-sarah-archive/)
- **24 files** for an inactive SEWY agent (agents-sewy-archive/ + sewy-garage-original/)
...
This creates maintenance overhead and makes it unclear which files are authoritative. The agent-registry.yaml also references a "sewy-garage" agent that has no corresponding active directory structure.

## What Changes

- **REMOVED** `old/` directory entirely (~100 files, ~50MB)
- **CONSOLIDATED** All Sarah/Wranngle documentation into centralized location
- **CLEANED** Root directory to contain only active project infrastructure
- **UPDATED** agent-registry.yaml to remove inactive SEWY agent reference
- **DOCUMENTED** Decision log explaining the reorganization rationale

### Specific Actions

**Files to Delete (old/ directory):**
- `old/agents-sarah-archive/` - 8 files (superseded by agents/sarah/)
- `old/agents-sewy-archive/` - 12 files (no active development)
- `old/sewy-garage-original/` - 12 files (duplicate of above)
...
- `old/sarah-*.md` - 4 files (merged into tech-spec.md)
- `old/sarah-sms-tool.json` - 1 file (integrated into pipelines)

**Registry Updates:**
- Remove or mark inactive: `sewy-garage` agent entry (no agents/sewy-garage/ exists)
- Update comments to reflect current structure

**Files to Keep (active infrastructure):**
```
voice_ai_agents/
├── agents/sarah/          ✅ Production agent (3 files)

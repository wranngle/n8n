...
The system SHALL update Pipedrive CRM when a valid person ID is available, gracefully degrading on failure.

#### Scenario: CRM update with valid person ID
- **WHEN** dynamic_variables contains pipedrive_person_id > 0
- **THEN** the system creates a note on the Pipedrive person
...
- **WHEN** pipedrive_person_id is null, 0, or missing
- **THEN** the system skips CRM operations
- **AND** logs "No CRM ID - skipped" as a warning
- **AND** sets crm_updated=false

#### Scenario: Pipedrive API failure
- **WHEN** Pipedrive note creation or person update fails after 3 retries
- **THEN** the system continues processing other branches

#!/usr/bin/env node
/**
 * Fix Wranngle Lead Qualifier Agent
 *
 * Issues to fix:
 * 1. Missing call_direction handling in prompt
 * 2. Missing tool verbalization guardrail
 *
 * Note: This agent already has dynamic first_message with {{customer_first_name}}
 */
...
// System override for lead qualifier (primarily outbound)
...
This agent is primarily used for OUTBOUND lead qualification calls.

### Outbound Calls (call_direction = outbound) - DEFAULT BEHAVIOR
...
- You are following up on their interest in automation solutions

### Inbound Calls (call_direction = inbound)
If someone calls this number directly:
...
- If still failing, offer an alternative
...
async function main() {
  console.log('\n╔═══════════════════════════════════════════════════════════════╗');
  console.log('║     FIX WRANNGLE LEAD QUALIFIER AGENT                         ║');
  ...
  try {
    ...
    console.log(`  Current first_message: "${currentFirstMessage.substring(0, 60)}..."`);
    console.log(`  Current prompt length: ${currentPrompt.length} chars`);

    if (currentPrompt.includes('SYSTEM OVERRIDE')) {
      console.log('\n⚠ Agent already has SYSTEM OVERRIDE. Skipping.');
      return;
    }
    ...
    console.log('\nApplying fixes:');
    console.log('  1. Adding SYSTEM OVERRIDE for call direction');
    console.log('  2. Adding tool verbalization guardrail');
    console.log('  (Keeping existing dynamic first_message)');
    ...
    console.log('\n═══════════════════════════════════════════════════════════════');
    console.log('                    LEAD QUALIFIER FIX COMPLETE');
    console.log('═══════════════════════════════════════════════════════════════\n');

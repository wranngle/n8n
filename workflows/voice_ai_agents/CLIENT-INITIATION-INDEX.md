...
The Client Initiation Data Enhancement enables ElevenLabs voice agents (starting with Sarah Wranngle) to:
...
- ✅ Automatically integrate with Pipedrive for post-call enrichment

### How It Works

```
Call Initiated → ElevenLabs Webhook → n8n Lookup (Pipedrive + Sheets)
...
**Prerequisites:**
- n8n instance with Pipedrive + Google Sheets credentials configured
- ElevenLabs agent (Sarah Wranngle)
...
- `temp/sarah_updated_prompt.md` - Enhanced agent prompt with CONTEXT AWARENESS section

**Backups:**
- `temp/sarah_agent_backup_before_client_init.json` - Pre-enhancement backup
...
**What:** Create realistic test contacts for Pipedrive/Sheets
...
**Causes:**
1. Phone numbers not in Pipedrive or Sheets
...
└────────┬────────┘
    ...
    ↓         ↓
┌────────┐ ┌──────────┐
│Pipedrive│ │ Google   │  Parallel API calls (300ms timeout each)
│  CRM    │ │ Sheets   │
└────┬───┘ └───┬──────┘
     │         │
     └────┬────┘
...
**Variable Naming:** Generic (`customer_name` not `sarah_customer_name`) for multi-agent reusability

**Data Merging:** Field-level merge with priority: most recent timestamp > Pipedrive > Sheets
...
**n8n Workflows:**
- Post-call webhook for Pipedrive note creation
- Slack notification system
- Evaluation pipeline

**ElevenLabs Agents:**
- Sarah Wranngle (Lead Specialist) - Current

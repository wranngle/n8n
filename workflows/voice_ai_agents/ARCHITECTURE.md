...
|                              External Services                                      |
|  +----------+  +---------+  +----------+  +---------+  +----------+                |
|  | Pipedrive|  | Google  |  | Twilio   |  | Cal.com |  | SMTP2GO  |                |
...
Phone Call → Twilio → ElevenLabs Agent
         ...
         ┌────────────────┴────────────────┐
         ↓                                 ↓
   Pipedrive Lookup              Google Sheets Lookup
...
| Pipedrive: Lookup Person | httpRequest | CRM customer lookup by phone |
...
```json
{
  ...
  "dynamic_variables": {
    ...
    "secret__pipedrive_org_id": 67890,
...
|------|-----------|----------------|
| Gold | Pipedrive account_tier = Gold | VIP personalized |
...
1. **Pipedrive** (highest priority) - Real-time CRM data
2. **Google Sheets** - Historical call logs
3. **Defaults** - Fallback for unknown callers

### Merge Strategy

- Customer name: Pipedrive > Sheets > "there"
- Company: Pipedrive only
- Interaction count: Sheets only
- Account tier: Pipedrive > Calculated from interaction count
...
| Failure | Behavior |
|---------|----------|
| Pipedrive timeout | Continue with Sheets data |
| Sheets timeout | Continue with Pipedrive data |
...
| Timeout errors | External API slow | Check Pipedrive/Sheets status |
...
3. Test with curl: `curl -X POST https://n8n.wranngle.com/webhook/client-initiation-data -H "Content-Type: application/json" -d '{"caller_id":"+15551234567","agent_id":"agent_8001kdgp7qbyf4wvhs540be78vew"}'`

...
- `old/agents-sarah-archive/` - 8 superseded Sarah agent configuration files
- `old/agents-sewy-archive/` - 12 SEWY garage agent files (development-only, never production)
- `old/sewy-garage-original/` - 12 duplicate files
...
1. **Single Source of Truth**: Every component now has exactly ONE authoritative location:
   - Sarah Agent → `agents/sarah/` (tech-spec.md is comprehensive)
...
|----------|-------|-------------|
| Sarah archives | 8 in agents-sarah-archive/ | Superseded by agents/sarah/tech-spec.md |
| Sarah prompts | 4 standalone .md files | Consolidated into tech-spec.md |
| SEWY agent | 24 (archive + original) | No active development, agent-registry entry removed |
...
3. `agent-registry.yaml` - Removed inactive sewy-garage entry
...
- ✅ Verified agents/sarah/tech-spec.md contains all prompt information
...
- SEWY agent removed from agent-registry.yaml as it has no active development

#!/usr/bin/env node
/**
 * detect-hardcoded-keys.js
 *
 * PreToolUse:Write hook that detects hardcoded API keys in code files
 * and blocks with instructions to use environment variables.
 *
 * PATTERNS DETECTED:
...
async function main() {
  ...
  try {
    ...
    for (const { name, pattern, envVar, loadCode } of KEY_PATTERNS) {
      ...
      if (matches) {
        detectedKeys.push({ name, matches: matches.length, envVar, loadCode });
      }
    }

    if (detectedKeys.length > 0) {
      const report = detectedKeys.map(k =>
        `- ${k.name}: ${k.matches} occurrence(s) → Use ${k.envVar} or ${k.loadCode}`
      ).join('\n');
...
Credentials are stored in: ~/.claude/.env`
    ...
    logHook('detect-hardcoded-keys', 'Error', { error: e.message });
...
module.exports = { KEY_PATTERNS, CODE_EXTENSIONS };
